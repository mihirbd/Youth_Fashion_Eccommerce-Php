<?php
    if(isset($_GET['status'])){
        ...
        if($_GET['status']=='unpublished'){
            $message=$obj_super_admin->unpublished_manufacturer_by_id($manufacturer_id);
        }
        else if($_GET['status']=='published'){
            $message=$obj_super_admin->published_manufacturer_by_id($manufacturer_id);
        }
        else if($_GET['status']=='delete'){
            $message=$obj_super_admin->delete_manufacturer_by_id($manufacturer_id);
        }
    }

    $query_result=$obj_super_admin->select_all_manufacturer_info();
...
<div class="row-fluid sortable">		
    <div class="box span12">
        <div class="box-header" data-original-title>
            <h2><i class="halflings-icon user"></i><span class="break"></span>All Manufacturer info goes Here</h2>
            <div class="box-icon">
                    ...
                    <tr>
                        ...
                        <td class="center">
                            <?php if ($manufacturer_info['publication_status'] == 1) { ?>
                                    <a class="btn btn-success" href="?status=unpublished&&id=<?php echo $manufacturer_info['manufacturer_id']; ?>" title="Published">
                                        <i class="halflings-icon white arrow-up"></i>  
                                    </a>
                                <?php } else { ?>
                                    <a class="btn btn-danger" href="?status=published&&id=<?php echo $manufacturer_info['manufacturer_id']; ?>" title="Unpublished">
