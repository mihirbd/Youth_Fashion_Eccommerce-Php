<?php
    if(isset($_GET['status'])){
        ...
        if($_GET['status']=='unpublished'){
            $message=$obj_super_admin->unpublished_category_by_id($category_id);
        }
        else if($_GET['status']=='published'){
            $message=$obj_super_admin->published_category_by_id($category_id);
        }
        else if($_GET['status']=='delete'){
            $message=$obj_super_admin->delete_category_by_id($category_id);
        }
    }

    $query_result=$obj_super_admin->select_all_category_info();
                    ...
                    <tr>
                        ...
                        <td class="center">
                            <?php if ($category_info['publication_status'] == 1) { ?>
                                    <a class="btn btn-success" href="?status=unpublished&&id=<?php echo $category_info['category_id']; ?>" title="Published">
                                        <i class="halflings-icon white arrow-up"></i>  
                                    </a>
                                <?php } else { ?>
                                    <a class="btn btn-danger" href="?status=published&&id=<?php echo $category_info['category_id']; ?>" title="Unpublished">
